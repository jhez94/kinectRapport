...
namespace Microsoft.Samples.Kinect.BodyBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        {
            public static Body body1 { get; set; }
            public static Body body2 { get; set; }
            public static ulong body1Index { get; set; }
            public static ulong body2Index { get; set; }
            public static bool rapoor { get; set; }
        }
        ...
        private void Reader_FrameArrived(object sender, BodyFrameArrivedEventArgs e)
        {
            ...
            using (BodyFrame bodyFrame = e.FrameReference.AcquireFrame())
            {
                if (bodyFrame != null)
                {
                    ...
                    //If no bodies are tracked, reset the body indexes
                    int bodiesTracked = 0;
                    ...
                    foreach (Body body in this.bodies)
                    {
                        if (body.IsTracked)
                        {
                            //Find two bodies and take down their body tracking index, and store the body into a global
                            if (GBVar.body1Index == 0)
                            {
                                GBVar.body1Index = body.TrackingId;
                                GBVar.body1 = body;
                                
                            }
                            else if(GBVar.body2Index == 0 && body.TrackingId != GBVar.body1Index)
                            {

                                //If first body is on right, switch body's tracking ID so body1 is on left, and body2 is on right
                                if (GBVar.body1.Joints[JointType.SpineShoulder].Position.X > body.Joints[JointType.SpineShoulder].Position.X)
                                {
                                    GBVar.body2Index = GBVar.body1Index;
                                    GBVar.body2 = GBVar.body1; 
                                    GBVar.body1Index = body.TrackingId;
                                    GBVar.body1 = body;
                                }
                                else
                                {
                                    GBVar.body2Index = body.TrackingId;
                                    GBVar.body2 = body;
                                }
                            }
                            //To see the track'ed body's IDs, uncomment the folling two lines. 
                            //Console.WriteLine("Body1 ID:  " + GBVar.body1Index);
                            //Console.WriteLine("Body2 ID:  " + GBVar.body2Index);
                    ...
                    if (GBVar.body1 != null && GBVar.body2 != null)// && MyGlobalVariables.body1.IsTracked && MyGlobalVariables.body2.IsTracked)
                    {
                        ...
                        //Following Conditions too binary. Need analog feedback.
                        //if (Math.Abs(body1Orient) > 0.2) body1Attract = true;
                        //else body1Attract = false;
                        //if (Math.Abs(body2Orient) > 0.2) body2Attract = true;
                        //else body2Attract = false;
                        //GBVar.rapoor = body1Attract && body2Attract;
                ...
                using (DrawingContext dc = this.drawingGroup.Open())
                {
                    ...
                    foreach (Body body in this.bodies)
                    {
                        ...
                        if (body.IsTracked)
                        {
                            ...
                            this.DrawRapoor(GBVar.rapoor, jointPoints[JointType.SpineShoulder], dc);
        ...
        private void DrawRapoor(bool rapoor, Point trackPoint, DrawingContext drawingContext)
        {
            switch (rapoor)
            {
                case false:
                    drawingContext.DrawEllipse(this.handClosedBrush, null, trackPoint, 30, 30);
                    break;

                case true:
                    drawingContext.DrawEllipse(this.handOpenBrush, null, trackPoint, 30, 30);
                    break;
            }
        }
